...
async def movie_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for t in torrents:
            ...
            hash_value = t["hash"]
            buttons.append([
                InlineKeyboardButton(f"{quality}", callback_data=f"quality_{hash_value}_{title.replace(' ', '_')}")
            ])
...
# Quality selected handler (working original)
        ...
        if torrent_response.status_code == 200:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".torrent") as tf:
                ...
                await context.bot.send_document(
                    ...
                    caption="Upload this file to [webtor.io](https://webtor.io) or open in [aTorrent](https://play.google.com/store/apps/details?id=com.utorrent.client).",
                    parse_mode="Markdown",
                    disable_web_page_preview=True
                )
        else:
